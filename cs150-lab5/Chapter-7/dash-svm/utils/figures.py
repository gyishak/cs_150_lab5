...
def serve_pie_confusion_matrix(model, X_test, y_test, Z, threshold):
    ...
    data=[{"Title": "True Positive","Value":tp},
          {"Title": "True Negative","Value":tn},
          {"Title": "False Positive","Value":fp},
          {"Title": "False Negative","Value":tp}]

    trace0=dash_table.DataTable(
        columns=[{"id":col, "name":col} for col in ["Title", "Value"] ],
        data=data
    )
    return trace0
